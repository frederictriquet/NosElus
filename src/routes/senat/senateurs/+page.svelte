<script lang="ts">
	...
	// svelte-ignore state_referenced_locally
	let senators = $state(data.senators);
	// svelte-ignore state_referenced_locally
	...
	async function loadMore() {
		...
		try {
			const response = await fetch(`/senat/senateurs?${params.toString()}`, {
...
<div class="page-header">
	<h1 class="page-title">Sénateurs</h1>
	<p class="page-subtitle">{data.pagination.total} sénateurs du Sénat</p>
</div>
...
{:else}
	<div class="card-grid">
		{#each senators as senator (senator.id)}
			<ElectedCard
				id={senator.id}
				name={senator.fullName}
				photoUrl={senator.photoUrl}
				group={senator.group}
				subtitle={senator.profession || ''}
				type="senateur"
			/>
		{/each}
	</div>
